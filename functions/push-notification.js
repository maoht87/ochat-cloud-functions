...
if (
	functions.config().push &&
	functions.config().push &&
	functions.config().push.web.click_action
) {
...
exports.sendNotification = functions.database
	...
	.onCreate((data, context) => {
		...
		if (message.sender == 'system') {
			console.log(
				'not send push notification for message with system as sender'
			);
		...
		if (
			...
			console.log(
				'not send push notification because sendnotification is false'
			);
			return 0;
		}
		...
		return admin
			...
			.ref(path)
			.once('value')
			.then(function (instancesIdAsFbObj) {
