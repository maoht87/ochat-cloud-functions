...
function OChat() {
	$(
		function () {
			this.$signInButton = $('#ochat-sign-in-button');
			this.$signOutButton = $('#ochat-sign-out-button');
			this.$messageTextarea = $('#ochat-message');
			this.$createMessageButton = $('#ochat-create-message');
			this.$createMessageResult = $('#ochat-create-message-result');
			this.$messageListButtons = $('.message-list-button');
			this.$messageList = $('#ochat-message-list');
			this.$messageDetails = $('#ochat-message-details');

			this.$signInButton.on('click', this.signIn.bind(this));
			this.$signOutButton.on('click', this.signOut.bind(this));
			this.$createMessageButton.on(
				'click',
...
OChat.prototype.signIn = function () {
	firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider());
};

OChat.prototype.signOut = function () {
	firebase.auth().signOut();
};

OChat.prototype.onAuthStateChanged = function (user) {
	if (user) {
...
OChat.prototype.createMessage = function () {
...
OChat.prototype.listMessages = function (event) {
...
OChat.prototype.messageDetails = function (event) {
...
OChat.prototype.authenticatedRequest = function (method, url, body) {
